...
项目流程
-----------------------------------
首先通过C写算法部分的代码，而现在对.h文件有了更加深入的理解，即只声明而不划分内存，而GUI部分则无需要特别好的实现效率，于是便使用了PYQT的高开发效率的特性。先使用ERIC4提供的QTdesigner绘制框架再通过ERIC4向python编译，再用自己写的窗口类继承setui后，完成GUI设计流程。